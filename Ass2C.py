...
def main():
    ...
    # Function to download data from EODHD
    ...
    def get_data(ticker):
        try:
            ...
            data = client.download(ticker, start="2022-01-01", end=pd.Timestamp.today().strftime('%Y-%m-%d'))
            return data
        ...
        try:
            ...
            if sections["1. GARCH Analysis"]:
                try:
                    ...
                    # Set up and fit a basic GARCH(1,1) model
                    model = arch_model(train, vol='Garch', p=1, q=1, rescale=False)
                    model_fit = model.fit(disp='off')  # Fit the model to the training data
                    st.write(model_fit.summary())  # Display model summary
                    ...
                    st.error(f"Error in GARCH analysis: {str(e)}")
            ...
            if sections["2. Neural Network"]:
                try:
                    ...
                    # Feature Preparation: Use lagged returns as features
                    ...
                    # Store NN model parameters
                    st.session_state.nn_params = nn_model.get_params()

                    # Rolling Forecast for Neural Network
                    st.write("## Rolling Forecast")
                    st.write("### Rolling Forecast Implementation:")
                    st.markdown("""
            ...
            if sections["3. Model Comparison"]:
                try:
                    ...
                    if (st.session_state.garch_mse == 'N/A' or st.session_state.nn_mse == 'N/A'):
                    ...
                    elif (st.session_state.garch_predictions is not None and 
                          st.session_state.nn_predictions is not None and 
                          st.session_state.actual_volatility is not None):

                        # Calculate comparison metrics
                        actual_vol = st.session_state.actual_volatility
                        garch_mae = np.mean(np.abs(st.session_state.garch_predictions - actual_vol))
                        nn_mae = np.mean(np.abs(st.session_state.nn_predictions - actual_vol))
                        ...
                        # Plot comparison
                        fig, ax = plt.subplots(figsize=(10, 5))
                        ax.plot(actual_vol, label="Actual Volatility", color="blue")
